...
class NewInvoicePage extends StatefulWidget {
  ...
  @override
  _NewInvoicePageState createState() =>
      _NewInvoicePageState(merchantId: merchantId);
}
...
class _NewInvoicePageState extends State<NewInvoicePage> {
  ...
  String get preferredCoinCode {
    return merchant?.denomination ??
        Authentication.currentAccount.preferredCoinCode;
  }
  ...
  void _submit() {
    ...
    if (account.coins.length == 0 && account.fetchingCoins)
      ...
      Client.createInvoice(_price, preferredCoinCode,
              accountId: merchant?.accountId)
          .then((response) {
        if (response['success']) {
          var invoiceId = response['invoiceId'];
          Navigator.pushNamed(context, '/invoices/$invoiceId', arguments: {
            'redirectUrl': merchant != null
                ? (window.document as HtmlDocument).referrer
                : null,
            'merchant': merchant,
          }).then((_) {
            setState(() {
      ...
      Navigator.pushNamed(context, '/settings/addresses').then((result) {
        _submitting = false;
        _rebuild();
      });
  }
    ...
    return Container(
        ...
        child: Text(merchant.name,
            ...
            style: TextStyle(
              ...
              fontSize:
                  (65 - 1.5 * max(merchant.name.length - 8, 0)).toDouble(),
            )));
  }

  Widget _VisiblePrice() {
    return Container(
      child: Text(_price > 0 ? '$_visiblePrice' : "",
          style: TextStyle(
                      ...
                      : GestureDetector(
                          ...
                          child: Container(
                            margin: EdgeInsets.only(
                                top: 10.0,
                                left: AppController.leftPadding() + 30),
                            child: Icon(Icons.menu,
                                size: 20 + AppController.scale(20.0)),
                          )),
                  Image(
  ...
  void _setInvoice() {
    _invoice = Invoice(
      denominationCurrency:
          merchant?.denomination ?? Authentication.currentAccount?.denomination,
  ...
  Widget _generateButton({text, onTap, font, icon}) {
    return GestureDetector(
        ...
        child: Container(
            ...
            width: AppController.scale(100, maxValue: 240, minValue: 100)
                .floor()
                .toDouble(),
            child: icon == null
                ? Text(text,
                ...
                : Align(
                    ...
                    child: Container(
                        margin:
                            EdgeInsets.only(top: AppController.scale(8) - 2),
                        child: Image.asset(icon,
                            width: 10 + AppController.scale(24))))));
  }
